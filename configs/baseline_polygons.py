...
config = Config()
config.rnd_seed = 0
config.description = f"DESP polygons {config.rnd_seed}"
config.num_epoch = 1
config.anomaly_detection = False
config.n_process_pool = 24
config.model = "baseline_digits_and_polygons"
config.train_loss = "lap"  # "lap"
config.eval_loss = "lap"

config.log = Config()
config.log.dir = Path("log_polygons")
...
config.data = Config()
config.data.name = "Polygons"
config.data.n_points = 8
config.data.n_poly = list(range(4,9))
...
config.energy_fn.d_x = len(config.data.n_poly)
